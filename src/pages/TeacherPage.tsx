...
export const allTeachers = fallbackTeachers.map(({ id, name, role, short_role }) => ({
  id,
  name,
...
export default function TeacherPage({ teacherId, onNavigate }: TeacherPageProps) {
  ...
  const [teacher, setTeacher] = useState<TeacherRecord | null>(fallbackTeacher || null);
      ...
      <div className="min-h-screen pt-28 pb-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="w-24 h-24 rounded-full bg-gradient-to-br from-gold/20 to-gold-dark/20 border border-gold/30 flex items-center justify-center mx-auto mb-6">
            <span className="font-playfair font-bold text-4xl text-gold">?</span>
          </div>
          <h1 className="font-playfair text-4xl font-light text-foreground mb-3">Педагог не найден</h1>
          <p className="text-muted-foreground font-golos">Проверь id или вернись в раздел преподавателей.</p>
        </div>
          ...
          {teacher && (
            <>
              <div className="flex flex-col sm:flex-row gap-8 items-start mb-12">
                <TeacherAvatar teacher={teacher} square className="w-32 h-32 flex-shrink-0" />
                <div>
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    <h1 className="font-playfair text-3xl md:text-4xl font-light text-foreground">{teacher.name}</h1>
                  </div>
                  <p className="text-gold font-golos text-sm mb-3">{teacher.short_role || teacher.role}</p>
              ...
              <div className="space-y-5 mb-10">
                {teacher.bio?.length ? teacher.bio.map((paragraph, i) => (
                  <p key={i} className="font-golos text-muted-foreground leading-relaxed">{paragraph}</p>
                )) : (
                  <p className="font-golos text-muted-foreground leading-relaxed">Подробная информация о педагоге скоро появится.</p>
                )}
              </div>
